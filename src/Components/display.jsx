...
export default class Display extends Component {
  ...
  iconEnter = (id) => {
    this.setState({ icon: true, noteId: id });
  };
  iconLeave = () => {
    this.setState({ icon: false, noteId: "" });
  };
  ...
  render() {
    ...
    const notes = Getnotes.filter((item) => item.isDeleted === false)
      .filter((item) => item.isArchived === false)
      .map((data, index) => {
        if (String(data.imageUrl).includes("client")) {
          data.imageUrl = data.imageUrl.replace("client", "");
        }
        return (
          <div
            onMouseEnter={() => this.iconEnter(data.id)}
            onMouseLeave={() => this.iconLeave(data.id)}
          >
            <Card
              key={data.id}
              className="card"
            ...
            >
                      ...
                      return (
                        <div className="getCollaborator">
                          <Tooltip title={colabData.email} placement="bottom">
                            <Avatar alt={colabData.firstName} src="/"></Avatar>
                          </Tooltip>
                        </div>
                      );
                    })
              ...
              <div
                className={
                  this.state.noteId === data.id ? "geticonhover" : "geticon"
                }
              >
                <Icons
                  className="iconclass"
